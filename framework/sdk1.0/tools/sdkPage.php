...
class sdkPage{
	...
	*/
	...
	function adminShow()
	{
		$page = $this->page_links();
		if ($page['total_page'] > 1) {
			$content.= '共有'.$this->total_nums.'条记录，当前第'.$page['current_page'].'/'.$page['total_page'].'页';
			$content.= '
			<ul class="pageLink">
				<li><a class="prev" href="'.$page['first_page'].'">首页</a></li>
				<li><a class="prev" href="'.$page['prew_page'].'">上一页</a></li>';
			
			foreach ($page['list'] as $n => $v){
				$content.= '<li>';
				if ($v['page'] == $page['current_page'] ){
					$content.= '<a class="nolink">'.$v['page'].'</a>';	
				} else {
					$content.= '<a href="'.$v['url'].'" >'.$v['page'].'</a>';
				}
				$content.= '</li>';
			}
			$content.= '								
				<li><a class="prev" href="'.$page['next_page'].'">下一页</a></li>
				<li><a class="prev" href="'.$page['last_page'].'">尾页</a></li>							
			</ul>';
		}
		return $content;
	}
